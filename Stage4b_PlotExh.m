...
clearvars -except filename filenamevec NITLvec ZHeAge ZFTAge AHeAge SARA_MODEL ITL_MODEL; close all; clc;

    filename=cell2mat(filenamevec(j));

    load(['./ComputeData/' filename '_' SARA_MODEL '_' ITL_MODEL '_zt.mat']); 
    load(['./computedata/' filename '_' SARA_MODEL '_' ITL_MODEL '_ESRfitpar.mat']);

    %% Define parameters for plotting %%
    DoI   = 5;                                 % Depth of Interest, last x km
    ToI   = 1;                                 % Time of Interest, last x Ma, was 0.15
    G_cut = 100;                               % Cut-off for geothermal gradient
    time_window = ToI;                         % Restrict to a time-window of interest for plotting was 0.15+0.25
    ...
    vel_interval = 20;  % define velocity smoothing interval in ka 

    % Measured data values
    nN = zt.nNnat; sigmanN = zt.snNnat; 

    % inversion model output
    misOUT = zt.misfit; Zt = zt.temp; 
    time = zt.time; G_end = zt.G_end; MTemp=[1:2];
    nNs = zt.maxnNs; maxnNs = max(nNs,[],8); nt = length(maxnNs(1,:)); MTemp=[1:nt+1];
    AgeESR = (records.params.SARADeGy./(records.params.natDdot(1).*3600*24*365.25*1e3))./1000;
    ...
    % define box size
    time_max=max(time); time_min=time_max-time_window;  % limit matrix time to RoI using time_window
    ...
    % scaling for resampling the PDF
    ...
    xlabel('Misfit'); ylabel('Geothermal gradient (^oC/km)');
    ylim([0 G_cut+10]);
    ...
    %% Plot modelled vs measured n/N values 
    col = round(100*(max(misOUT(id)):(min(misOUT)-max(misOUT(id)))/6:min(misOUT)))/100;

    f2 = figure(2); 
    subplot(1,2,2); xlabel('ESR signal'); ylabel('n/N'); axis square; box on; hold on; 
    c1 = colorbar('location', 'northoutside'); ylabel(c1, 'Misfit'); colormap('viridis'); 
    ...
    legend([h2,h1],'nN_{obs}','nN_{pred}', 'Location','SouthEast'); 
    xlim([0 nt+1]); ylim([0.0 1]); hold off
    ...
    axis square; box on; hold on; c3 = colorbar('location', 'northoutside'); ylabel(c3, 'PDF'); colormap('viridis')  
    ...
    set(gca,'YDir','Reverse'); ylabel('Depth (km)'); xlabel('Time (Ma)'); 
...
%     text(plott,plotd2,filename,'fontweight','bold','col','w');
    ...
    %% Compute velocity of median model for all ac. path
    ...
    t_vel = t_median_intp(end:-vel_interval:1)'; % time-median value at every 5 ka  
    z_vel = z_median_intp(end:-vel_interval:1)'; % depth-median value at every 5 ka  
    ...
    for i = 1:resampling
        ...
        R_res         = randi(movea,1,nb_resPath);  % Choose randomly 10% of paths from accepted paths
    ...
    Vel_90CI_u  = sortedVel_res(:,round(0.90*resampling));  % upper bounds of 90% CI 
    Vel_90CI_l  = sortedVel_res(:,round(0.10*resampling));  % lower bounds of 90% CI
    Vel_95CI_u  = sortedVel_res(:,round(0.95*resampling));  % upper bounds of 95% CI   
    Vel_95CI_l  = sortedVel_res(:,round(0.05*resampling));  % upper bounds of 95% CI
    ...
    plotvec=zeros(length(AgeESR(:,1)),1); plotveclog=ones(length(AgeESR(:,1)),1).*0.1; 
    f3 = figure(3); 
    ...
    xlim([time_max-ToI time_max]); xlabel('Time [Ma]'); ylabel('Exhumation rate [mm/y]'); 
    ...
    xlabel('Time (Ma)'); xlabel('Time (Ma)'); 
    ylabel('Exhumation rate (mm/y)'); set(gca, 'YScale', 'log');  
    ...
    save(['./ComputeData/' filename '_' SARA_MODEL '_' ITL_MODEL '_zt.mat'],'zt'); 

    print(f1,'-depsc',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Geotherm.eps']);
    print(f2,'-depsc',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Depth_Ages.eps']);
    print(f3,'-depsc',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Vel_lin.eps']);
    print(f4,'-depsc',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Vel_log.eps']);
    
%     print(f1,'-dpng',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Geotherm.png']);
%     print(f2,'-dpng',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Depth_Ages.png']);
%     print(f3,'-dpng',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Vel_lin.png']);
%     print(f4,'-dpng',['./Figures/' filename '_' SARA_MODEL '_' ITL_MODEL  '_Vel_log.png']);
