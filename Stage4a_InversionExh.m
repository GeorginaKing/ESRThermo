...
timemaxESR   =tmax;                                     % fix ESR data to end of model, i.e. 5 Ma = present day
nstepESR     =timemaxESR*1000;                          % ensure high resolution nstep
ESR_length   =1;                                        % time in Ma over which TL modelled
...
%% Import Kinetic data
NNnat = reshape([records.params.SARAnNnat],2,nt)'; NNnat=NNnat(ntmin:ntmax,:); 
nN = NNnat(:,1); sigmanNnat = NNnat(:,2)%.*0.05; % define arbitrary unceratinty on n/Nnat values
